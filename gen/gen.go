...
type genTypeWriter func(*Config, *spec.Swagger, *swag.Parser) error
...
func (g *Gen) Build(config *Config) error {
	...
	for _, outputType := range config.OutputTypes {
		...
		if typeWriter, ok := g.outputTypeMap[outputType]; ok {
			if err := typeWriter(config, swagger, p); err != nil {
				return err
			}
		} else {
...
func (g *Gen) writeDocSwagger(config *Config, swagger *spec.Swagger, parser *swag.Parser) error {
...
func (g *Gen) writeJSONSwagger(config *Config, swagger *spec.Swagger, parser *swag.Parser) error {
...
func (g *Gen) writeMDSwagger(config *Config, swagger *spec.Swagger, parser *swag.Parser) error {
...
func (g *Gen) writeYAMLSwagger(config *Config, swagger *spec.Swagger, parser *swag.Parser) error {
